...
bool AsmAnalyzer::operator()(Identifier const& _identifier)
{
	...
	bool success = true;
	if (m_currentScope->lookup(_identifier.name, dev::GenericVisitor{
...
bool AsmAnalyzer::operator()(ExpressionStatement const& _statement)
{
	...
	bool success = visit(*this, _statement.expression);
...
bool AsmAnalyzer::operator()(Assignment const& _assignment)
{
	...
	bool success = visit(*this, *_assignment.value);
...
bool AsmAnalyzer::operator()(VariableDeclaration const& _varDecl)
{
	...
	{
		int const stackHeight = m_stackHeight;
		success = visit(*this, *_varDecl.value);
	...
	for (auto const& variable: _varDecl.variables)
	{
		...
		m_activeVariables.insert(&get<Scope::Variable>(m_currentScope->identifiers.at(variable.name)));
	}
...
bool AsmAnalyzer::operator()(FunctionDefinition const& _funDef)
{
	...
	for (auto const& var: _funDef.parameters + _funDef.returnVariables)
	{
		...
		m_activeVariables.insert(&get<Scope::Variable>(varScope.identifiers.at(var.name)));
	}
...
bool AsmAnalyzer::operator()(FunctionCall const& _funCall)
{
	...
	else if (!m_currentScope->lookup(_funCall.functionName.name, dev::GenericVisitor{
	...
	for (auto const& arg: _funCall.arguments | boost::adaptors::reversed)
	{
		...
		else if (needsLiteralArguments)
		{
			...
			else if (!m_dataNames.count(get<Literal>(arg).value))
				m_errorReporter.typeError(
					_funCall.functionName.location,
					"Unknown data object \"" + get<Literal>(arg).value.str() + "\"."
				);
		}
	}
...
bool AsmAnalyzer::checkAssignment(Identifier const& _variable, size_t _valueSize)
{
	...
	if (Scope::Identifier const* var = m_currentScope->lookup(_variable.name))
	{
		...
		else if (!m_activeVariables.count(&get<Scope::Variable>(*var)))
		{
			m_errorReporter.declarationError(
				_variable.location,
