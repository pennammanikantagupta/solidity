...
void ForLoopConditionOutOfBody::operator()(ForLoop& _forLoop)
{
	...
	if (
		...
		valueOfLiteral(get<Literal>(*_forLoop.condition)) == u256(0) ||
		_forLoop.body.statements.empty() ||
		!holds_alternative<If>(_forLoop.body.statements.front())
	)
		return;

	If& firstStatement = get<If>(_forLoop.body.statements.front());
	if (
	...
	if (
		...
		get<FunctionCall>(*firstStatement.condition).functionName.name == iszero
	)
		_forLoop.condition = make_unique<Expression>(std::move(get<FunctionCall>(*firstStatement.condition).arguments.front()));
	else
