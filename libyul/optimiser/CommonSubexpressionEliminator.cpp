...
void CommonSubexpressionEliminator::visit(Expression& _e)
{
	...
	if (holds_alternative<FunctionCall>(_e))
		if (BuiltinFunction const* builtin = m_dialect.builtin(get<FunctionCall>(_e).functionName.name))
	...
	{
		Identifier& identifier = get<Identifier>(_e);
		...
		if (m_value.count(name))
		{
			...
			{
				YulString value = get<Identifier>(*m_value.at(name)).name;
