...
void LoadResolver::visit(Expression& _e)
{
	...
	{
		FunctionCall const& funCall = get<FunctionCall>(_e);
	...
	{
		FunctionalInstruction const& instruction = get<FunctionalInstruction>(_e);
...
)
{
	...
	YulString key = get<Identifier>(_arguments.at(0)).name;
	if (
