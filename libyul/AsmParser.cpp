...
Statement Parser::parseStatement()
{
	...
	case Token::AssemblyAssign:
	{
		...
		while (true)
		{
			...
			auto const& identifier = get<Identifier>(elementary);
		...
		Assignment assignment =
			createWithLocation<Assignment>(get<Identifier>(elementary).location);
	...
	case Token::Colon:
	{
		...
		Identifier const& identifier = get<Identifier>(elementary);
	...
	if (holds_alternative<Identifier>(elementary))
	{
		Identifier& identifier = get<Identifier>(elementary);
		return ExpressionStatement{identifier.location, { move(identifier) }};
	}
	if (holds_alternative<Literal>(elementary))
	{
		Expression expr = get<Literal>(elementary);
		return ExpressionStatement{locationOf(expr), expr};
	}
	else
	{
		solAssert(holds_alternative<Instruction>(elementary), "Invalid elementary operation.");
		return get<Instruction>(elementary);
	}
}
...
Case Parser::parseCase()
{
	...
	else if (currentToken() == Token::Case)
	{
		...
		_case.value = make_unique<Literal>(get<Literal>(std::move(literal)));
	}
	else
...
Expression Parser::parseExpression()
{
	...
	else if (holds_alternative<Instruction>(operation))
	{
		...
		Instruction const& instr = get<Instruction>(operation);
...
Expression Parser::parseCall(Parser::ElementaryOperation&& _initialOp)
{
	...
	if (holds_alternative<Instruction>(_initialOp))
	{
		...
		Instruction& instruction = get<Instruction>(_initialOp);
	...
	else if (holds_alternative<Identifier>(_initialOp) || holds_alternative<FunctionCall>(_initialOp))
	{
		...
		{
			ret.functionName = std::move(get<Identifier>(_initialOp));
			ret.location = ret.functionName.location;
		}
		else
			ret = std::move(get<FunctionCall>(_initialOp));
		expectToken(Token::LParen);
		if (currentToken() != Token::RParen)
		{
			ret.arguments.emplace_back(parseExpression());
